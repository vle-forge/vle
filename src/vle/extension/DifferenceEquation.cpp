                              ...
                              const Time& time)
{
    ...
    while (it != event.end()) {
        if (mMode == NAME and (*it)->onPort("update")) {
            ...
            processUpdate(name, **it, begin, end);
        }
...
void Generic::externalTransition(const devs::ExternalEventList& event,
                                 const devs::Time& time)
{
    ...
    Simple::externalTransition(event, time);
}
