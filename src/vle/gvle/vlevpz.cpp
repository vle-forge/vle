...
vleVpz::vleVpz(const QString& filename) : hasMeta(false),
    mFilename(filename), mFileNameVpm(""), mDoc("vle_project"), mDocVpm(0),
    mVdo(0), mVdoVpm(0), undoStack(0), undoStackVpm(0),
    waitUndoRedoVpz(false), waitUndoRedoVpm(false),
    oldValVpz(), newValVpz(), oldValVpm(), newValVpm(), mLogger(0),
    mGvlePlugins(0), maxPrecision(std::numeric_limits<double>::digits10)
{
    xReadDom();
...
vleVpz::vleVpz(QXmlInputSource& source): hasMeta(false),
    mFilename(""), mFileNameVpm(""), mDoc("vle_project"), mDocVpm(0),
    mVdo(0), mVdoVpm(0), undoStack(0), undoStackVpm(0),
    waitUndoRedoVpz(false), waitUndoRedoVpm(false),
    oldValVpz(), newValVpz(), oldValVpm(), newValVpm(), mLogger(0),
    mGvlePlugins(0), maxPrecision(std::numeric_limits<double>::digits10)
...
vleVpz::vleVpz(const QString& vpzpath, const QString& vpmpath,
        gvle_plugins* plugs): hasMeta(true),
    mFilename(vpzpath), mFileNameVpm(vpmpath), mDoc("vle_project"), mDocVpm(0),
    mVdo(0), mVdoVpm(0), undoStack(0), undoStackVpm(0),
    waitUndoRedoVpz(false), waitUndoRedoVpm(false),
    oldValVpz(), newValVpz(), oldValVpm(), newValVpm(), mLogger(0),
    mGvlePlugins(plugs), maxPrecision(std::numeric_limits<double>::digits10)
{
    xReadDom();
    mVdo = new vleDomVpz(&mDoc);
    undoStack = new DomDiffStack(mVdo);
    ...
    QObject::connect(vleVpz::undoStack,
                ...
                this, SLOT(onUndoRedoStackVpz(QDomNode, QDomNode)));
    QObject::connect(undoStackVpm,
                SIGNAL(undoRedoVdo(QDomNode, QDomNode)),
                this, SLOT(onUndoRedoStackVpm(QDomNode, QDomNode)));
    QObject::connect(vleVpz::undoStack,
...
{
    QDomNode classesElem = DomFunctions::obtainChild(
            vleProjectFromDoc(), "classes", &mDoc);
        ...
        const QString& destinationPort)
{
    ...
    for (int j=0; j<conList.length(); j++) {
        ...
        if ((DomFunctions::attributeValue(con.toElement().elementsByTagName("origin")
                .item(0), "model") == sourceModel) and
            (DomFunctions::attributeValue(con.toElement().elementsByTagName("origin")
                .item(0), "port") == sourcePort) and
            (DomFunctions::attributeValue(con.toElement().elementsByTagName("destination")
...
vleVpz::atomicModelFromModel(const QDomNode& node, const QString& atom) const
{
    ...
    if (currSubModelTag.nodeName() != "submodels") {
        if ((DomFunctions::attributeValue(node, "type") == "atomic")
                and (DomFunctions::attributeValue(node, "name") == atom)) {
            return node;
        }
        return res;
    }
...
{
    if (not existObsFromDoc(oldName) or existObsFromDoc(newName)
        or oldName == newName) {
        return;
    }
        ...
        const QString& oldPortName, const QString& newPortName)
{
    ...
    QDomNode portToRename = DomFunctions::childWhithNameAttr(obs, "port",
            oldPortName);
...
vleVpz::renameViewToDoc(const QString& oldName, const QString& newName)
{
    ...
    QList<QDomNode> outList = DomFunctions::childNodesWithoutText(outputs, "output");
    ...
    QList<QDomNode> obsList = DomFunctions::childNodesWithoutText(obss, "observable");
    for (int i =0; i< obsList.size(); i++) {
        QDomNode o = obsList[i];
        QList<QDomNode> portList = DomFunctions::childNodesWithoutText(o, "port");
        for (int j =0; j< portList.size(); j++) {
            QDomNode p = portList[j];
            QList<QDomNode> attViewL = DomFunctions::childNodesWithoutText(p, "attachedview");
...
{
    return DomFunctions::childNameProvider(dynamicsFromDoc(),
            "dynamic", prefix);
}
...
{
    return DomFunctions::childNameProvider(viewsFromDoc(),
                "view", prefix);
}
...
{
    return DomFunctions::childNameProvider(condsFromDoc(),
                "condition", prefix);
}
...
vleVpz::newCondPortNameToDoc(QString cond, QString prefix) const
{
    ...
    return DomFunctions::childNameProvider(condAtom,
                "port", prefix);
}
...
vleVpz::rmViewToDoc(const QString& viewName)
{
    ...
    QList<QDomNode> obsList = DomFunctions::childNodesWithoutText(observables, "observable");
    for (int i=0; i<obsList.size(); i++) {
        QDomNode o = obsList[i];
        QList<QDomNode> portList = DomFunctions::childNodesWithoutText(o, "port");
        for (int j=0; j<portList.size(); j++) {
            QDomNode p = portList[j];
            QList<QDomNode> atList = DomFunctions::childNodesWithoutText(p, "attachedview");
...
{
    QDomNode added = vleDomStatic::addCond(condsFromDoc(), condName, &mDoc,
            undoStack);
    if (not added.isNull()) {
        synchronizeUndoStack();
        emit conditionsUpdated();
...
{
    vleDomStatic::rmPortFromCond(condFromConds(condsFromDoc(), condName),
            portName, undoStack);
}
...
vleVpz::rmObservableFromModel(QDomNode &node, const QString &obsName)
{
    ...
    for (int j=0; j < list.length(); j++) {
        ...
        if (item.nodeName() == "submodels") {
            ...
            if(DomFunctions::attributeValue(currModel, "observables") == obsName) {
...
vleVpz::renameModel(QDomNode node, const QString& newName)
{
    ...
    if (node.parentNode().nodeName() == "structures") {
        modif = vleDomStatic::renameModelIntoStructures(mDoc,
                node.parentNode(),
                oldName, newName, undoStack);
    } else {
        modif = vleDomStatic::renameModelIntoCoupled(mDoc,
        ...
        for (int i=0; i< conList.length(); i++) {
            ...
            QDomNode dest = con.toElement()
             ...
             if ((DomFunctions::attributeValue(con, "type") == "input")  and
                ...
                (DomFunctions::attributeValue(orig, "model") == modelName)) {
                toRemove.append(con);
            } else if ((DomFunctions::attributeValue(con, "type") == "internal")  and
                    ((DomFunctions::attributeValue(orig, "model") == modelName) or
                     (DomFunctions::attributeValue(dest, "model") == modelName))) {
                toRemove.append(con);
            }
        }
...
vleVpz::existSubModel(QDomNode node, const QString& modName)
{
    ...
    QList<QDomNode> chs = DomFunctions::childNodesWithoutText(node, "submodels");
    if (chs.size() == 1) {
        QList<QDomNode> mods = DomFunctions::childNodesWithoutText(chs[0], "model");
        for (int i=0; i<mods.size(); i++) {
...
vleVpz::existSiblingModel(QDomNode node, const QString& modName)
{
    ...
    QList<QDomNode> mods = DomFunctions::childNodesWithoutText(node.parentNode(), "model");
    for (int i=0; i<mods.size(); i++) {
        if (DomFunctions::attributeValue(mods[i], "name") == modName) {
            return true;
        }
...
vleVpz::addModel(QDomNode node, const QString& type, QPointF pos)
{
    ...
    if (type == "coupled") {
        DomFunctions::setAttributeValue(newModel,"height","50");
        DomFunctions::setAttributeValue(newModel,"width","50");
        DomFunctions::setAttributeValue(newModel,"y",
                QVariant(pos.y()).toString());
        DomFunctions::setAttributeValue(newModel,"x",
        ...
        for (int i=0; i<modsToCopy.size(); i++) {
            ...
            while (not found) {
                ...
                if (not existSubModel(modDest, subName)) {
                    ...
                    DomFunctions::setAttributeValue(sub, "x",
                            QVariant(DomFunctions::attributeValue(sub, "x").toDouble()
                                    + xtranslation).toString());
                    DomFunctions::setAttributeValue(sub, "y",
                            QVariant(DomFunctions::attributeValue(sub, "y").toDouble()
            ...
            QDomNode subs = DomFunctions::obtainChild(
                    modDest, "submodels",&mDoc);
        ...
        for (int i =0; i<consOrig.size(); i++) {
            ...
            if (DomFunctions::attributeValue(con, "type") == "internal") {
                ...
                QDomNode destPort = con.toElement()
                        .elementsByTagName("destination").at(0);
                if (storeOrigNames.contains(DomFunctions::attributeValue(origPort, "model"))
                    ...
                    DomFunctions::setAttributeValue(origPort, "model",
                            storeCopyNames[idOrig]);
                    DomFunctions::setAttributeValue(destPort, "model",
            ...
            for (int i=0; i< conList.size(); i++) {
                ...
                if (portType == "in" ) {
                    if ((DomFunctions::attributeValue(con, "type") == "input") or
                            (DomFunctions::attributeValue(con, "type") == "internal")) {
                        QDomNode dest = con.toElement()
                                                .elementsByTagName("destination").at(0);
                        if ((DomFunctions::attributeValue(dest, "model") == modName) and
                                (DomFunctions::attributeValue(dest, "port") == oldName)) {
                    ...
                    if ((DomFunctions::attributeValue(con, "type") == "output") or
                        ...
                        QDomNode orig = con.toElement()
                                                .elementsByTagName("origin").at(0);
                        if ((DomFunctions::attributeValue(orig, "model") == modName) and
                                (DomFunctions::attributeValue(orig, "port") == oldName)) {
        ...
        for (int i=0; i< conList.length(); i++) {
            ...
            if (portType == "in" ) {
                if ((DomFunctions::attributeValue(con, "type") == "input") or
                    ...
                    QDomNode dest = con.toElement()
                            .elementsByTagName("destination").at(0);
                    if ((DomFunctions::attributeValue(dest, "model") == ownerModel) and
                            (DomFunctions::attributeValue(dest, "port") == oldName)) {
                ...
                if ((DomFunctions::attributeValue(con, "type") == "output") or
                    ...
                    QDomNode orig = con.toElement()
                            .elementsByTagName("origin").at(0);
                    if ((DomFunctions::attributeValue(orig, "model") == ownerModel) and
                            (DomFunctions::attributeValue(orig, "port") == oldName)) {
                        rmModelConnection(con, false);
                    }
                }
...
vleVpz::addModelPort(QDomNode node, const QString& type)
{
    ...
    while (not found) {
        found = true;
        for (int i=0; i< ports.length(); i++) {
            if (DomFunctions::attributeValue(ports.at(i), "name") == newPortName) {
                found = false;
            }
        }
        if (not found) {
        ...
        QString connType)
{
    ...
    for (int i =0; i<currConns.length(); i++) {
        ...
        if (currCon.nodeName() == "connection" and
            ...
            if ((DomFunctions::attributeValue(currOrig, "model") == modelOrig) and
                    (DomFunctions::attributeValue(currDest, "model") == modelDest) and
                    (DomFunctions::attributeValue(currOrig, "port") == portOrig) and
...
vleVpz::setHeightToModel(QDomNode node, int height)
{
    ...
    DomFunctions::setAttributeValue(node,"height", QVariant(height).toString());
}
        ...
        QList<int> ys)
{
    ...
    if (size > 0) {
        ...
        for (int i =0; i<size; i++) {
            QDomNode nodei = nodes[i];
            DomFunctions::setAttributeValue(nodei, "x", QVariant(xs[i]).toString());
            DomFunctions::setAttributeValue(nodei, "y", QVariant(ys[i]).toString());
        }
    }
}
...
vleVpz::buildOutputConfigMap(const QString& outputName)
{
    ...
    QList<QDomNode> nodes = DomFunctions::childNodesWithoutText(outputNode, "map");
    if (nodes.size() == 1) {
...
vleVpz::setTimeStepToDoc(const QString& viewName, double ts)
{
    ...
    if (getViewTypeFromDoc(viewName).contains("timed") and ts > 0.0) {
        undoStack->snapshot(viewsFromDoc());
        synchronizeUndoStack();
        DomFunctions::setAttributeValue(node, "timestep",
...
vleVpz::fillWithClassesFromDoc(std::vector<std::string>& toFill)
{
    ...
    for (int i=0; i< classNodes.size(); i++) {
        toFill.push_back(DomFunctions::attributeValue(
...
vleVpz::importFromClipboard(QDomNode node_dest)
{
    ...
    QString vpmFile = QDir::tempPath() +"/clipboard/metadata/exp/clipboard.vpm";
    ...
    QSet<QString> elems = modifyImportSourceForImport(node_dest,
            queries, track, src, src_mod_query);
    if (elems.empty()) {
        return ;
    }
    ...
    for (QSet<QString>::iterator i = elems.begin(); i!=elems.end(); i++) {
        QStringList elem = i->split("::");
        if (elem.at(0) == "cond") {
            condsFromDoc().appendChild(src.condFromConds(src.condsFromDoc(),
                    elem.at(1)));
            emit_cond = true;
        } else if (elem.at(0) == "dyn") {
            dynamicsFromDoc().appendChild(src.dynamicFromDoc(
                    elem.at(1)));
            emit_dyn = true;
        } else if (elem.at(0) == "obs") {
            obsFromDoc().appendChild(src.obsFromObss(src.obsFromDoc(),
                    elem.at(1)));
            emit_obs = true;
            ...
            QDomNode src_cond_node = src.condFromConds(src.condsFromDoc(), src_cond);
            QDomNode src_port_node = src.portFromCond(src_cond_node, src_cond_port);
    ...
    if (src_mod_query != "") {
        //import models
        QDomNode dest_submodels = DomFunctions::obtainChild(node_dest,
                        "submodels", &mDoc);
        ...
        for (QSet<QString>::iterator i = elems.begin(); i!=elems.end(); i++) {
            QStringList elem = i->split("::");
            if (elem.at(0) == "model") {
                QDomNode model_node = vleDomStatic::subModel(src_mod,
                        elem.at(1));
        ...
        //import connections
        QDomNode dest_connections = DomFunctions::obtainChild(node_dest,
                "connections", &mDoc);
        QDomNode src_connections = DomFunctions::obtainChild(src_mod,
                        "connections");
        QList<QDomNode> cons = DomFunctions::childNodesWithoutText(
                src_connections, "connection");
        for (int i =0; i<cons.size(); i++) {
            QDomNode con = cons.at(i);
            if (DomFunctions::attributeValue(con, "type") == "internal") {
                if (mods_to_import.contains(vleDomStatic::connectionModOrig(con))
                    and mods_to_import.contains(vleDomStatic::connectionModDest(con))) {
...
vleVpz::exportToClipboard(QList<QDomNode> nodes_source, bool track)
{
    ...
    for (int i = 0; i<nodes_source.length(); i++) {
        clip_content += "query:="+vdo()->getXQuery(nodes_source.at(i))+";;";
    }
    clip_content+="track=";
    if (track) {
        clip_content+="yes";
...
vleVpz::saveTemp(QString pkg, QString prefix)
{
    ...
    QString vpmFile = QDir::tempPath() +"/"+pkg+"/metadata/exp/"+prefix+".vpm";
    saveVpz(vpzFile);
    saveVpm(vpmFile);
}
...
{
    int undoAvailability = DomDiffStack::computeUndoAvailability(
            prevCurr, curr, saved);
    ...
    for (int i=0; i<import_queries.size(); i++) {
        ...
        if (src_node_name == "condition") {
            ...
            if (not importing_cond) {
                condsToExclude.unite(vleDomStatic::conditions(
                        src.condsFromDoc()));
                importing_cond = true;
            }
            QString dest_cond_new = src_cond;
            if (not DomFunctions::childWhithNameAttr(
                    condsFromDoc(), "condition", src_cond).isNull()) {
                dest_cond_new = DomFunctions::childNameProvider(condsFromDoc(),
                                    "condition", src_cond, condsToExclude);
                src.renameConditionToDoc(src_cond, dest_cond_new);
                condsToExclude.insert(dest_cond_new);
                log_messages.append(QString(
                        "condition '%1' is imported as '%2'")
                        .arg(src_cond).arg(dest_cond_new));
            }
            elems.insert(QString("cond::%1").arg(dest_cond_new));
        } else if((src_node_name == "port") and
                (src_node.parentNode().nodeName() == "condition")) {
            QString src_cond_port = DomFunctions::attributeValue(
                    src_node, "name");
            QString src_cond = DomFunctions::attributeValue(
                    src_node.parentNode(), "name");
            if (dest_node_name != "condition") {
                elems.clear();
                break;
            }
            QString dest_cond = DomFunctions::attributeValue(dest_node, "name");
            if (getCondGUIplugin(dest_cond) != "") {
                elems.clear();
                break;
            }
            if (not importing_cond_ports) {
                condPortsToExclude.unite(DomFunctions::childNames(
                        src_node.parentNode(), "port"));
                importing_cond_ports = true;
            }
            QString dest_cond_port_new = src_cond_port;
            if (not DomFunctions::childWhithNameAttr(
                    dest_node, "port", src_cond_port).isNull()) {
                dest_cond_port_new = DomFunctions::childNameProvider(
                        dest_node, "port", src_cond_port, condPortsToExclude);
                src.renameCondPortToDoc(src_cond, src_cond_port,
                        dest_cond_port_new);
                condPortsToExclude.insert(dest_cond_port_new);
                log_messages.append(QString("condition port '%1' from condition"
                        " '%2' is imported as '%3'").arg(src_cond_port)
                        .arg(dest_cond).arg(dest_cond_port_new));
            }
            elems.insert(QString("cond_port::%1//%2//%3")
                    .arg(src_cond).arg(dest_cond).arg(dest_cond_port_new));
        } else if (src_node_name == "model") {
            if ((dest_node_name != "model") or (DomFunctions::attributeValue(
                    dest_node, "type") != "coupled")) {
                elems.clear();
                break;
            }
            ...
            QString src_mod_name = DomFunctions::attributeValue(
                    src_node, "name");
            if (not importing_models) {
                modelsToExclude.unite(DomFunctions::childNames(
                        src_node.parentNode(), "model"));
                condsToExclude.unite(vleDomStatic::conditions(
                        src.condsFromDoc()));
                obsToExclude.unite(vleDomStatic::observables(
                        src.obsFromDoc()));
                dynToExclude.unite(vleDomStatic::dynamics(
                        src.dynamicsFromDoc()));
                importing_models = true;
            }
            QString dest_mod_new = src_mod_name;
            if (not DomFunctions::childWhithNameAttr(
                    DomFunctions::obtainChild(dest_node, "submodels"), "model",
                    src_mod_name).isNull()) {
                dest_mod_new = DomFunctions::childNameProvider(
                        DomFunctions::obtainChild(dest_node, "submodels"),
                        "model", src_mod_name, modelsToExclude);
                ...
                log_messages.append(QString("model '%1' is imported as '%2'")
                        .arg(src_mod_name).arg(dest_mod_new));
            }
            ...
            if (track) {
                ...
                for (int j =0; j<src_conds_att.size(); j++) {
                    ...
                    QString dest_cond_new = src_cond;
                    if (not DomFunctions::childWhithNameAttr(condsFromDoc(),
                            "condition", src_cond).isNull()){
                ...
                QString src_obs_att = vleDomStatic::attachedObsToAtomic(
                        src_node);
                if (src_obs_att != "") {
                    QString dest_obs_new = src_obs_att;
                    if (not DomFunctions::childWhithNameAttr(
                            obsFromDoc(), "observable",src_obs_att).isNull()){
                        dest_obs_new = DomFunctions::childNameProvider(
                                obsFromDoc(), "observable", src_obs_att,
                                obsToExclude);
                        src.renameObservableToDoc(src_obs_att,
                                dest_obs_new);
                        obsToExclude.insert(dest_obs_new);
                        log_messages.append(QString(
                                "observable '%1' is imported as '%2'")
                                .arg(src_obs_att).arg(dest_obs_new));
                    }
                ...
                if (src_dyn_att != "") {
                    ...
                    if (not DomFunctions::childWhithNameAttr(dynamicsFromDoc(),
                            "dynamic",src_dyn_att).isNull()){
                        dest_dyn_new = DomFunctions::childNameProvider(
                                dynamicsFromDoc(), "dynamic", src_dyn_att,
                                dynToExclude);
                        src.renameDynamicToDoc(src_dyn_att, dest_dyn_new);
                        dynToExclude.insert(dest_dyn_new);
                        log_messages.append(QString(
                                "dynamic '%1' is imported as '%2'")
                                .arg(src_dyn_att).arg(dest_dyn_new));
                    }
