...
Translator::Translator(xmlpp::Element* root) :
    ...
    m_root(root)
{
    Assert(Exception::Parse, root, "Translator with null XML element.");
}
						...
						long value)
{
    ...
    newnode->set_attribute("VALUE",utils::ucompose("%1",value));
    return newnode;
}
				    ...
				    double value)
{
    ...
    newnode2->set_attribute("VALUE", utils::ucompose("%1", value));
}
				...
				int index)
{
    ...
    if (index != -1)
	observableNode->set_attribute("INDEX",utils::ucompose("%1",index));
}
