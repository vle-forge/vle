/**
 * @file src/vle/data/CSVResult.cpp
...
namespace vle { namespace data {
    ...
    CSVResult::CSVResult(const std::string& request,
			 ...
			 unsigned int columnNumber):request(request),
						    csv(csv),type(type),
	...
	if (colNum == 0) {
	  ...
	  selectionNameMap[value] = std::pair < int,bool>(colNum,distinct);	  
	}
	else {
	...
	if (colNum > 0) {
	  ...
	  if (type != CSVReader::MATRIX) {
	    ...
	    if (distinct) 
	      distinctMap[columnNameList[colNum-1]] = std::list<std::string>();
	  }
	  else {
	    selectionNameMap[valueList[colNum-columnNumber-1]] = 
	      std::pair < int,bool>(colNum,distinct);
	    if (distinct) 
	      distinctMap[valueList[colNum-columnNumber-1]] = std::list<std::string>();
	  }
	}
	++its;
      }
	...
	if (csv.getline(str) != 0) {
      ...
      std::map < std::string, std::list < std::string > >::iterator it = distinctMap.begin();
	...
	if (type == CSVReader::MATRIX)
	  more = csv.getline(str) != 0 && 
	    csv.getnfield() == (int)(columnNameList.size()+valueList.size());
	else
	  more = csv.getline(str) != 0 && 
	    csv.getnfield() == (int)columnNameList.size();
	if (more) {
	  std::vector < int >::iterator it1 = conditionIndexList.begin();
	  std::vector < std::string >::iterator it2 = conditionValueList.begin();
	  ...
	  if (it1 == conditionIndexList.end()) {
		...
		std::string value = csv.getfield(it->second.first-1);
		    std::list < std::string >::iterator itd = find(distinctMap[it->first].begin(),distinctMap[it->first].end(),value);
	    ...
	    while (ok and it1 != conditionIndexList.end()) {
	      ...
	      // vérifie que la valeur trouvée n'est pas déjà selectionnée dans
	      // le cas de distinct
	...
	// Mise à zéro des listes de valeurs distinctes
	std::map < std::string, std::list < std::string > >::iterator it5 = distinctMap.begin();
