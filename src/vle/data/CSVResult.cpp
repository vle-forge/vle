...
CSVResult::CSVResult(const std::string& request,
    ...
    : request(request), csv(csv), type(type), columnNumber(columnNumber)
{
    std::string str;
    int colNumber = 0;
    ...
    if (not selectionList.empty()) {
        ...
        for (unsigned int i = 1; i <= columnNumber; ++i) {
            this->selectionList.push_back(i);
        }
    }
    ...
    do {
        if (type == CSVReader::MATRIX) {
            ...
            if (type == CSVReader::TITLE) {
                ...
                more = csv.getline(str) != 0 and
                    csv.getnfield() == (int)columnNumber;
            }
        }
        if (more) {
